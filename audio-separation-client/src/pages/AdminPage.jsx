...
export default function AdminPage({ user, onNavigate, onLogout }) {
  ...
  const handleEditUser = (u) => {
    setModalData({ id: u.Id, email: u.Email, password: "", role: u.Role });
    setModal("editUser");
  };
  ...
  const handleSaveUser = async () => {
    try {
      ...
      if (modal === "createUser") {
        ...
        await api.updateUser(user.id, modalData.id, modalData.email, modalData.password, modalData.role);
                      ...
                      <tbody>
                        {users.map((u) => (
                          <tr key={u.Id} className="border-b border-white/10 hover:bg-white/5 transition">
                            <td className="py-4 px-4 text-white/70">{u.Id}</td>
                            ...
                            <td className="py-4 px-4 text-white/60 text-sm">{formatDate(u.CreatedAt)}</td>
                    ...
                    {guests.map((g) => (
                      <div
                        ...
                        className={`bg-white/5 border rounded-lg p-4 transition ${
                          isExpired(g.ExpiresAt) ? "border-red-500/30" : "border-white/20"
                        }`}
                      >
                        <div className="flex justify-between items-start mb-3">
                          ...
                          {isExpired(g.ExpiresAt) && (
                            <span className="bg-red-500/30 text-red-200 text-xs px-2 py-1 rounded">
                              Expirat
                            </span>
                          )}
                        </div>
                        ...
                        <div className="space-y-2 text-sm mb-4">
                          ...
                          <div className="flex justify-between text-white/60">
                            <span>Expiră:</span>
                            <span className={isExpired(g.ExpiresAt) ? "text-red-400" : "text-white/80"}>
                          ...
                          <div className="mt-3">
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="text-white/80">
                                {g.UsedToday}/{g.DailyLimit}
                              </span>
                            </div>
                            <div className="w-full bg-white/10 rounded-full h-2">
                              <div
                                className="bg-gradient-to-r from-purple-500 to-pink-500 h-2 rounded-full transition"
                                style={{
                                  width: `${(g.UsedToday / g.DailyLimit) * 100}%`,
                                }}
                              ></div>
                            </div>
                          </div>
                        ...
                        {jobs.map((j) => (
                          <tr key={j.Id} className="border-b border-white/10 hover:bg-white/5 transition">
                            ...
                            <td className="py-4 px-4 text-white text-sm">
                              {j.UserEmail ? (
                                <span className="bg-blue-500/20 px-2 py-1 rounded text-blue-200">
                                  👤 {j.UserEmail}
                                </span>
                              ) : (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-white/20 rounded-2xl p-6 max-w-md w-full">
            <h3 className="text-2xl font-bold text-white mb-4">
              {modal === "createUser" ? "➕ Adaugă Utilizator" : "✏️ Editează Utilizator"}
            </h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2 text-white placeholder-white/40 focus:outline-none focus:border-purple-500"
                  placeholder={modal === "editUser" ? "(lăsați gol dacă nu schimbați)" : "Introduceți parola"}
                />
              </div>
